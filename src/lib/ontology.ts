...
const FALLBACK: Omit<OntologySnapshot, 'live'> = {
  ...
  actions: [
    ...
    {
      key: 'action_create_work_package',
      label: 'Create work package',
      description: 'Adds a unit of scope to the project — a discipline, a cost and hours estimate, and planned dates. Starts in DRAFT.',
    ...
    {
      key: 'action_update_work_package',
      label: 'Update work package',
      description: 'Progresses a work package and books actuals against it. Omitted fields are left unchanged. Refuses to move to COMPLETE while open NCRs are linked to it.',
      target_type: 'WORK_PACKAGE',
      cascades: [],
      is_agent_usable: true,
      parameters: [
        ...
        { name: 'p_actual_end', type: 'date' },
      ],
    },
    {
      key: 'action_schedule_inspection',
      label: 'Schedule inspection',
      description: 'Books a survey attendance against a work package, before it happens. Result stays PENDING until it is recorded.',
      target_type: 'INSPECTION_EVENT',
      cascades: [],
      is_agent_usable: true,
      parameters: [
    ...
    {
      key: 'action_link_defect_to_work_package',
      label: 'Attach NCR to work package',
      description: 'Attaches an open NCR to the work package it affects, or detaches it when the work package is omitted. A closed NCR cannot be re-linked.',
      target_type: 'DEFECT_RECORD',
      cascades: [],
      is_agent_usable: true,
      parameters: [
